...
export default function AcademyDashboard() {
  ...
  async function loadData() {
    ...
    try {
      ...
      const { academy: acData, coaches: allCoaches } = await res.json();

      setAcademy(acData ?? null);
      const pending  = (allCoaches ?? []).filter((c: Coach) => c.status === "pending");
      const approved = (allCoaches ?? []).filter((c: Coach) => c.status === "approved");
      setPendingCoaches(pending);
      setApprovedCoaches(approved.map((c: Coach) => ({ ...c, players: [], expanded: false })));
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  async function handleApprove(coachId: string) {
    ...
    await fetch("/api/academy/dashboard", {
      method:  "PATCH",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify({ coachId, status: "approved" }),
    });
  ...
  async function handleReject(coachId: string) {
    ...
    await fetch("/api/academy/dashboard", {
      method:  "PATCH",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify({ coachId, status: "rejected" }),
    });
    setActionLoading(null);
    loadData();
  }
  ...
  async function handleRemoveCoach(coachId: string) {
    ...
    setActionLoading(coachId);
    try {
      await fetch(`/api/auth/academy/coaches/${coachId}`, { method: "DELETE" });
    } catch (e) {
      console.error(e);
  ...
  async function handleRemovePlayer(playerId: string, coachId: string) {
    ...
    try {
      const res = await fetch(`/api/academy/dashboard/players/${playerId}`, { method: "DELETE" });
  ...
  async function toggleCoachExpand(coachId: string) {
    ...
    if (!coach?.expanded && (!coach?.players || coach.players.length === 0)) {
      const res = await fetch(`/api/academy/dashboard/players?coachId=${coachId}`);
      if (res.ok) {
        const { players } = await res.json();
        setApprovedCoaches((prev) =>
          prev.map((c) => c.id === coachId ? { ...c, players: players ?? [], expanded: true } : c)
        );
      }
    }
  }
  ...
  return (
    <>
      ...
      <main className="container animate-fade-in" style={{ paddingTop: "40px", paddingBottom: "60px" }}>
        ...
        <div className="glass-card" style={{ display: "inline-flex", gap: "4px", padding: "4px", borderRadius: "12px", marginBottom: "24px", border: "1px solid rgba(245,158,11,0.2)" }}>
          {TABS.map((t) => (
            <button
              key={t.key}
            ...
            {activeTab === "pending" && (
              <div>
                {pendingCoaches.length === 0 ? (
                  <div className="glass" style={{ padding: "32px", textAlign: "center", color: "var(--text-secondary)" }}>No pending coach requests.</div>
                ) : (
                    ...
                    {pendingCoaches.map((coach) => (
                      <div key={coach.id} className="glass-card" style={{ padding: "20px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
                        <div>
                          ...
                          <div style={{ color: "var(--text-secondary)", fontSize: "12px", marginTop: "4px" }}>Requested {new Date(coach.created_at).toLocaleDateString()}</div>
                        </div>
            ...
            {activeTab === "coaches" && (
              <div>
                {approvedCoaches.length === 0 ? (
                  <div className="glass" style={{ padding: "32px", textAlign: "center", color: "var(--text-secondary)" }}>No approved coaches yet. Approve coaches from the Pending tab.</div>
                ) : (
                  <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                      ...
                      <div key={coach.id} className="glass-card" style={{ padding: "0", overflow: "hidden" }}>
                        ...
                        >
                          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                            <div style={{ width: "38px", height: "38px", borderRadius: "8px", background: "linear-gradient(135deg,#6366f1,#818cf8)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: "700", fontSize: "16px" }}>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); handleRemoveCoach(coach.id); }}
                              disabled={actionLoading === coach.id}
                          ...
                          <div style={{ borderTop: "1px solid var(--border-subtle)", padding: "16px 24px", background: "var(--surface-1)" }}>
                            {!coach.players || coach.players.length === 0 ? (
                              ...
                              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                                {coach.players.map((p) => (
                                  <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", borderRadius: "8px", background: "var(--surface-2)" }}>
                                    <div>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                                      <span style={{ fontSize: "11px", fontWeight: "700", padding: "3px 10px", borderRadius: "20px", background: p.status === "approved" ? "rgba(16,185,129,0.12)" : "rgba(245,158,11,0.12)", color: p.status === "approved" ? "var(--success)" : "var(--warning)" }}>
                                        {p.status}
                                      </span>
                                      <button
                                        onClick={() => handleRemovePlayer(p.id, coach.id)}
                                        disabled={removePlayerLoading === p.id}
                                        style={{ background: "rgba(239,68,68,0.1)", color: "var(--danger)", border: "none", padding: "5px 10px", borderRadius: "6px", cursor: removePlayerLoading === p.id ? "not-allowed" : "pointer", fontSize: "12px", fontWeight: "700", opacity: removePlayerLoading === p.id ? 0.6 : 1 }}
                                      >
            ...
            {activeTab === "overview" && academy && (
              <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                <div className="glass-card" style={{ padding: "24px" }}>
                  ...
                  {academy.city && <p style={{ color: "var(--text-secondary)", marginBottom: "8px" }}>📍 {academy.city}</p>}
                  {academy.description && <p style={{ color: "var(--text-secondary)", lineHeight: "1.6" }}>{academy.description}</p>}
                </div>
